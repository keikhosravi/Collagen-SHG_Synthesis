...
import torch
import pandas as pd
from skimage import io, transform, img_as_float, color, exposure, data
import numpy as np
...
#         input[:, :, 1] = output
#         input[:, :, 2] = output
#         input = color.rgb2hsv(input)
#         input[:, :, 2] = exposure.adjust_gamma(input[:, :, 2])
        input = exposure.equalize_adapthist(input, clip_limit=0.03)
        output = output-0.012
        output = output.clip(min=0)
        output = exposure.adjust_gamma(output, 0.4)
